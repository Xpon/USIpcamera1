...
public class CodecManager {
	...
	public synchronized static Codec[] findEncodersForMimeType(String mimeType) {
		...
		Log.e(TAG,"MediaCodecList.getCodecCount="+MediaCodecList.getCodecCount());
		for(int j = MediaCodecList.getCodecCount() - 1; j >= 0; j--){
			MediaCodecInfo codecInfo = MediaCodecList.getCodecInfoAt(j);
			if (!codecInfo.isEncoder()) continue;

			String[] types = codecInfo.getSupportedTypes();
			Log.e(TAG,"types.length="+types.length);
			for (int i = 0; i < types.length; i++) {
				Log.e(TAG,"type="+types[i]);
				if (types[i].equalsIgnoreCase(mimeType)) {
					try {
						...
						Log.e(TAG,"capabilities.colorFormats.length="+capabilities.colorFormats.length);
						for (int k = 0; k < capabilities.colorFormats.length; k++) {
							int format = capabilities.colorFormats[k];
							Log.e(TAG,"format="+format);

							for (int l=0;l<SUPPORTED_COLOR_FORMATS.length;l++) {
								Log.e(TAG,"SUPPORTED_COLOR_FORMATS="+SUPPORTED_COLOR_FORMATS[l]);
								if (format == SUPPORTED_COLOR_FORMATS[l]) {
									Log.e(TAG,"---format="+format+"=="+"SUPPORTED_COLOR_FORMATS="+SUPPORTED_COLOR_FORMATS[l]+"---");
									formats.add(format);
								}
							}
						}
						Log.e(TAG,"codecInfo.getName()="+codecInfo.getName()+";"+"formats.size()="+formats.size());
