...
public class EncoderDebugger {
	...
	private void debug() {
		...
		for (int i=0;i<encoders.length;i++) {
			Log.e(TAG,"encoders.length="+encoders.length+";"+"formats.length="+encoders[i].formats.length);
			for (int j=0;j<encoders[i].formats.length;j++) {
				reset();
				
				mEncoderName = encoders[i].name;
				mEncoderColorFormat = encoders[i].formats[j];
				Log.e(TAG,"mEncoderName="+mEncoderName+";"+"mEncoderColorFormat="+mEncoderColorFormat);
